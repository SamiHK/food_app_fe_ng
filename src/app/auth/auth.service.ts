...
export class AuthService {
  ...
  forgetPassword(body: {username: string}) {
    return this.http.post(`${this.BASE_URL}/forget/password`, body)
    .pipe(
      catchError(this.commonService.catchError)
    );
  }
  ...
  resetPassword(token: string, body: {password: string, confirmPassword: string}) {
    return this.http.post(`${this.BASE_URL}/reset/password/${token}`, body)
    .pipe(
      catchError(this.commonService.catchError)
    );
  }
  ...
  enabled(id: string, params: { enabled: boolean }) {
    return this.http.get(`${this.BASE_URL}/${id}/enabled/`, {
      params: params
    })
    .pipe(
      catchError(this.commonService.catchError)
